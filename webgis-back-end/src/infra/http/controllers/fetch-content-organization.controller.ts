...
@Controller('/fetch-content-organization')
export class FetchContentOrganizationController {
  constructor(
    private fetchContentOrganizationUseCase: FetchContentOrganizationUseCase,
  ...
  async handle(@CurrentUser() user?: UserPayload) {
    ...
    const result = await this.fetchContentOrganizationUseCase.executeMany();
    ...
    const camadasVisiveis = this.filterVisiveis(
      enrichedCamadas,
      ctx,
      (i) => i.grupoCamada,
      (i) => i.idUsrCriacao,
    );
    const camadasRasterVisiveis = this.filterVisiveis(
      enrichedCamadasRaster,
      ctx,
      (i) => i.grupoCamada,
      (i) => i.idUsrCriacao,
    );
    const mapasVisiveis = this.filterVisiveis(
      enrichedMapas,
      ctx,
      (i) => i.grupoMapa,
      (i) => i.idUsrCriacao,
    ...
    return {
      ...
      camadasRaster: camadasRasterVisiveis,
      mapas: mapasVisiveis,
    };
  }

  private filterVisiveis(
    itens: any[],
    ctx: GrupoAccessContext,
    grupoIdDe: (item: any) => string,
    idUsrCriacaoDe: (item: any) => string,
  ): any[] {
    return itens.filter((item) =>
      this.grupoAccessPolicy.canViewItemByNivel(
        ctx,
        item.nivelCompartilhamento,
        grupoIdDe(item),
        idUsrCriacaoDe(item),
      ),
    );
