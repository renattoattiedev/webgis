import { HttpClient, HttpHeaders } from '@angular/common/http';
import { Injectable } from '@angular/core';
import { CookieService } from 'ngx-cookie-service';
import { Observable, of, catchError, throwError } from 'rxjs';
import { Folder } from '../../models/folders.camadas.model';
import { GetConfigService } from '../get.config.service';

@Injectable({
  providedIn: 'root',
})
export class RecoveryUserService {
  constructor(
    private http: HttpClient,
    private cookieService: CookieService,
    private getConfigService: GetConfigService,
  ) {}

  recoveryUser(userId: string): Observable<any> {
    const baseUrl = this.getConfigService.getUrl('recovery-user');

    const token = this.cookieService.get('access_token');
    if (token && baseUrl !== 'undefined') {
      const headers = new HttpHeaders({
        Authorization: `Bearer ${token}`,
      });
      const url = `${baseUrl}/${userId}`;
      return this.http.put<Folder>(url, {}, { headers }).pipe(
        catchError((error) => {
          console.error('Erro ao recuperar o usuário:', error);
          return throwError(() => new Error('Erro ao recuperar o usuário'));
        }),
      );
    } else {
      console.error('Token não encontrado no cookie ou URL inválida');
      return of(null);
    }
  }
}
