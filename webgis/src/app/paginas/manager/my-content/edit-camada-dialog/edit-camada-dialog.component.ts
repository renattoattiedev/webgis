...
@Component({
  ...
  templateUrl: './edit-camada-dialog.component.html',
  styleUrls: ['./edit-camada-dialog.component.scss'],
})
export class EditCamadaDialogComponent implements OnInit {
  temas: Tema[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<EditCamadaDialogComponent>,
    ...
    this.camadaForm = this.fb.group({
      ...
      fonte_dados: [this.data.camada.fonteDadosCamada],
    });
  }
  ...
  onSubmit() {
    if (this.camadaForm.valid) {
      this.updateCamadasService.updateCamada(this.data.camada).subscribe({
        next: (response) => {
          console.log('Camada atualizada com sucesso!', response);
          this.salvarGruposAdicionais();
        },
        error: (error) => {
          console.error('Erro ao atualizar a camada', error);
          this.snackBar.open(
            `❌ ${error?.message || 'Erro ao atualizar a camada'}`,
            'Fechar',
            { duration: 5000 },
          );
        },
  ...
  private salvarGruposAdicionais(): void {
    ...
    const chamadas = [
      ...adicionados.map((id) =>
        this.gruposService.adicionarItem(id, 'camada', this.data.camada.id),
      ),
      ...removidos.map((id) =>
        this.gruposService.removerItem(id, 'camada', this.data.camada.id),
      ),
    ];
    ...
    forkJoin(chamadas).subscribe({
      ...
      error: (error) => {
        console.error('Erro ao atualizar grupos adicionais da camada', error);
        this.snackBar.open(
          '❌ Camada salva, mas houve erro ao atualizar grupos adicionais',
          'Fechar',
          { duration: 4000 },
        );
        this.dialogRef.close(true);
